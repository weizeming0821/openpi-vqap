...
class LeRobotRLBenchDataConfig(DataConfigFactory):
    """
    Data config for the converted RLBench LeRobot dataset.

    The current M0 baseline keeps the same camera layout as pi0.5 pretraining:
    front -> base_0_rgb, wrist -> left_wrist_0_rgb, right_wrist_0_rgb -> zeros.
    """
    ...
    @override
    def create(self, assets_dirs: pathlib.Path, model_config: _model.BaseModelConfig) -> DataConfig:
        # Repack the dataset column names into the keys expected by RLBenchInputs.
        repack_transform = _transforms.Group(
            inputs=[
                _transforms.RepackTransform(
                    {
                        "observation/front": "observation.images.front",
        ...
        # Convert RLBench raw pose/image fields into the common pi input format.
        data_transforms = _transforms.Group(
